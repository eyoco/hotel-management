...
include 'connect.php';
$conn = OpenCon();
$sql = "SELECT AVG(price), numberParticipants
		FROM eventBooking
		GROUP BY numberParticipants";
$result = $conn->query($sql);

if($result->num_rows > 0) {
	echo "<table><tr><th class='border-
class'>average price</th><th class='border-
class'>numberParticipants</th></tr>";	 
 // output data of each row
 while($row = $result->fetch_assoc()) 
 { echo
	"<tr><td class='border-
class'>".$row["AVG(price)"]."</td><td class='border-
class'>".$row["numberParticipants"]."</td></tr>";
 }
 echo "</table>";
} else {
 echo "0 results";
