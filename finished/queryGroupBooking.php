...
include 'connect.php';
$conn = OpenCon();
$sql = "SELECT groupid, numberParticipants, numberRoomNeeded
FROM groupBooking";
$result = $conn->query($sql);

if($result->num_rows > 0) {
 echo "<table><tr><th class='border-
class'>groupid</th><th class='border-
class'>numberParticipants</th><th class='border-
class'>numberRoomNeeded</th></tr>"; 
 // output data of each row
 while($row = $result->fetch_assoc()) 
 { echo
 "<tr><td class='border-
class'>".$row["groupid"]."</td><td
class='border-
class'>".$row["numberParticipants"]."</td><td
class='border-
class'>".$row["numberRoomNeeded"]."</td></tr>";
 }
 echo "</table>";
} else {
 echo "0 results";
